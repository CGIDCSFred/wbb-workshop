...
proof of equivalence, spec-driven extension, and a spec-bounded L3 support
chatbot — adapting it to TD IT constraints (no Docker, restricted network,
possibly no live LLM API).
...
to choose; what must transfer is the meaning, the data model, the four planted
...
3. Author the artifacts in **§6** so the four inconsistencies in **§5** are
...
| **No Anthropic API key, or LLM API egress blocked** | Run **offline-first** (§11): pre-generate the spec, regeneration, FAQ, operational-history, and new-report content as static files at *build* time, and ship a *scripted* chatbot. The running demo then needs **no network at all**. Treat live generation as an optional enhancement only if a key + egress exist. |
...
regenerate the system, and durable enough to serve as an L3 support knowledge
base.*

The arc: **live system → forensic spec → regeneration from spec alone → proof of
equivalence → new feature from spec → production-support knowledge.**

It is delivered as a 10-tab Streamlit app run as a 6-act presentation (§10, §15).

The domain is entirely synthetic — no TD data or IP. It is *structurally* like a
real modernisation (legacy ETL → warehouse, drifted docs) so the method
transfers, without copying anything real.
...
```
wbb-workshop/
  app.py                      # the 10-tab Streamlit app (the whole demo)
  ...
  artifacts/                  # the inputs the reverse-engineering reads
    brd_wbb_v1.1.md
    source_schema.sql
    target_schema.sql
    ...
    etl/ wbbxtr.py  wbbldr.py  wbb_common.py
  prompts/
...
CREATE TABLE onboarding_applications (
    ...
    decided_at TEXT,                     -- set when APPROVED/DECLINED
    assigned_to TEXT
);
...
## 5. The four seeded inconsistencies — the calibration (reproduce EXACTLY)
...
1. **D1 — Date column drift.** The BRD (§5) states weekly volume metrics are
   ...
   date key (`submitted_year_week`, derived from `submitted_at`/`submitted_dt`).
   The warehouse reporting view `vw_weekly_onboarding_volume` counts by
   submission date, not approval date. Both dates exist on the source; the wrong
   one is used as the primary.
...
Three of the seven ServiceNow tickets (§6) map onto D1, D2, D3, D4 — "the spec
predicted the tickets."
...
the system and discover D1–D4. Keep them realistic and internally *plausible*,
...
- **`source_schema.sql`** — operational DDL using `business_category` (D2), the
  applications table with `submitted_at` and `decided_at`.
...
- **`etl/wbbldr.py`** (load) — writes the dimension and fact, keyed on submission
  week (D1), and **does not write `decline_description`** (D3 part 2).
- **`etl/wbb_common.py`** — shared helpers (connection, surrogate keys, week
  derivation).
- **`servicenow_tickets_wbb.json`** — 7 WBB-domain incidents; **4 map to D1–D4**
  (e.g. "weekly dashboard counts by submission not approval", "decline_description
  column missing", "nightly job fails at audit step — wbbaudit not found",
  "which column is the segment?"), 3 are unrelated open issues.
...
## 7. The three prompts (verbatim)

Reproduce these as `prompts/01..03`. The five rules and seven sections of prompt
01 are fixed.
...
(Attach the 8 artifacts: BRD, both schemas, user stories, job config, and the
three ETL files.)
...
### ⚠️ Surrogate keys MUST be deterministic across processes

Do **not** use Python's built-in `hash()` — it is randomized per process
(`PYTHONHASHSEED`). If the dim is loaded in one process and the fact in another
(any app restart), keys diverge and the fact→dim join silently breaks, making
the equivalence proof show false non-equivalence. Use `hashlib`:

```python
import hashlib
def surr_key_orig(prefix, id_):   # original ETL
    return int(hashlib.sha256(f"orig|{prefix}|{id_}".encode()).hexdigest(), 16) % (2**63)
def surr_key_regen(prefix, id_):  # regenerated ETL — different VALUES, same guarantee
...
| 2 | **Artifacts** | Render the six core artifacts (BRD, schemas, user stories, job config, ETL files) for browsing. |
| 3 | **Spec** | Show the forensic spec. **Live mode:** "Generate Spec Live" streams prompt 01 + the 8 artifacts, side-by-side with the pre-built reference (equal columns; capture reference text *before* overwriting; no rerun so both stay for comparison). **Offline mode:** show `fallback/wbbaw_spec_v1.md`. Always show the "4 discrepancies" callout. |
...
| 7 | **Production Tickets** | Render the 7 ServiceNow incidents; highlight the 4 mapped to D1–D4. |
| 8 | **Enriched Spec** | Spec + an operational-history "Section 8" (ticket patterns folded into the spec). |
| 9 | **FAQ** | FAQ generated from the enriched spec (decline_description gap, segment naming, audit-step failure, etc.). |
| 10 | **L3 Chatbot** | Spec-bounded assistant. **Live mode:** Anthropic call with a system prompt = enriched spec + Section 8, instructed to answer ONLY from the spec and classify each ticket as **KNOWN PATTERN** / **KNOWN GAP** / **UNKNOWN — ESCALATE**, citing spec sections. **Offline mode:** a scripted classifier that pattern-matches the known demo tickets to canned KNOWN PATTERN / KNOWN GAP / UNKNOWN responses (see §11). |

UI: TD-blue theme, wide layout. Tabs 1, 2, 5–9 are fully offline; 3, 4, 10 use the
API only in live mode.
...
1. **At build time** (you, the building Claude, with the operator): produce the
   ...
   operational history, and the FAQ — by running the prompts in §7 yourself —
   and save them as files under `fallback/`. These become the demo's content.
...
1. **Deterministic surrogate keys** (§8). Never built-in `hash()`. This is the
   one bug that silently breaks the equivalence proof — the climax of the demo.
...
4. Author the **artifacts** (§6) with D1–D4 planted exactly (§5).
5. Write the **prompts** (§7).
6. **Generate the content** for offline-first mode (§11) and save to `fallback/`.
7. Build the **10 tabs** (§10) reading the static content; add live paths if a
   key exists (with graceful fallback, §12).
8. Theme it (TD blue), wire the 6-act flow.
...
- [ ] **Tab 5 → Compare shows a green EQUIVALENT badge**, *and* the
      ...
      and re-Compare — still green; this catches the `hash()` bug).
- [ ] The spec (Tab 3) reconstructs **all four** D1–D4 with provenance citations,
      in the 7-section structure, ending cleanly.
...
- [ ] Tabs 7–9 render tickets / enriched spec / FAQ; 4 tickets map to D1–D4.
- [ ] Tab 10 classifies the demo tickets correctly (live or scripted) and never
      shows a traceback.
- [ ] No live tab crashes when the network/API is unavailable — it degrades to
      the fallback with a friendly notice.
- [ ] The four inconsistencies are **present and unfixed** in the artifacts.

---

## 15. The six-act talk track
...
- **Act 1 — Live system (Tabs 1–2, ~3 min).** "This is the source system —
  ...
  are the delivery artifacts. They look complete — but does the code do what the
  BRD says?"
- **Act 2 — Forensic reverse engineering (Tab 3, ~6 min).** Generate (or show)
  ...
  the code as evidence, each with a citation."
...
- **Act 5 — New feature (Tab 6, ~3 min).** "Average days to approval, from the
  ...
  record."
- **Act 6 — Production knowledge (Tabs 7–10, ~6 min).** Tickets (4 map to
  D1–D4 — "the spec predicted them") → enriched Section 8 → FAQ → the
...
- **Close.** "The WBB domain is fictional; the method transfers directly to a
  real WBB→TDBC stream: gather artifacts, reverse-engineer a forensic spec where
  every claim has a source and every contradiction is preserved, prove it
  regenerates, then use it to onboard, validate before cutover, and run L3
  support — one auditable document from delivery through production."
